...
SDL_AppResult SDL_AppInit(void **appstate, int argc, char *argv[]) {
    ...
    SDL_GPUShader *vertex_shader = LoadShader(device, "shader.vert", 0, 1, 0, 0);
    if (!vertex_shader) {
    ...
    SDL_GPUBufferRegion vertex_buffer_region = {
        ...
        vertex_buffer_region.size = sizeof(vertex_data),
    };
...
SDL_AppResult SDL_AppIterate(void *appstate) {
    ...
    if (!SDL_WaitAndAcquireGPUSwapchainTexture(cmd_buf, window, &swapchain_texture, &drawablew,
                                               &drawableh)) {
    ...
    SDL_PushGPUVertexUniformData(cmd_buf, 0, matrix_final, sizeof(matrix_final));
    SDL_GPURenderPass *render_pass =
        SDL_BeginGPURenderPass(cmd_buf, &color_target_info, 1, &depth_target_info);

...
private:
  ...
  void BuildUIGeometry(const std::vector<Slot> &inventory, int inventorySlot,
                       Player &player);
...
public:
  ...
  ~Renderer() {
    ...
    if (QuadIndexBinding.buffer) {
      SDL_ReleaseGPUBuffer(this->basicInitVars.GPU, QuadIndexBinding.buffer);
      QuadIndexBinding = {nullptr, 0};
    }
  ...
  void DrawUI(const std::vector<Slot> &inventory, int inventorySlot,
              Player &player);

...
constexpr HeightsDiff ContinentelnessHeight[8] = {{1, ySize},
                                                  {0.4f, ySize * 0.9f},
                                                  {0.15f, ySize * 0.8f},
                                                  {-0.15f, ySize * 0.45f},
                                                  {-0.35f, ySize * 0.45f},
                                                  ...
                                                  {-0.9f, ySize * 0.09f},
                                                  {-1.0f, ySize}};
constexpr HeightsDiff ErrotionHeight[10] = {
...
constexpr HeightsDiff PeaksAndValiesHeight[6] = {
    {1, ySize},
    {0.8f, ySize * 0.9f},
    {0.5f, ySize * 0.95f},
    {0.05f, ySize * 0.35f},
...
int ChunkManager::BaseHeight(float ValueNoise, int Length, const HeightsDiff* Heights) {
    for (int i = 0; i < Length - 1; i++) {
        if (ValueNoise > ContinentelnessHeight[i].x) {
            std::cout << "Continentalness: "
                      << Lerp(Heights[i].y, Heights[i + 1].y,
...
Biome ChunkManager::GetBiome(float Humudity, float Temperature) {
    ...
    for (int i = 0; i < 11; i++) {
        ...
        bool allowed_temperature =
            (Temperature < Biomes[i].MaxTemperature && Temperature > Biomes[i].MinTemperature);
...
namespace ChunckManager {
        ...
        bool Collition(Vector3& PlayerPos, int FullRange, int yRange, bool Swim, bool Block)
        {
                ...
                //std::cout << "Chunk: " << chunk << ", LocalX: " << localX << ", FootY: " << footY
<< ", HeadY: " << headY << ", Block Info: " << Chunks[chunk].Blocks[localX][footY] << std::endl;

                bool blockHead = !isTransparent(Chunks[{(int)chunk.x, (int)chunk.z}].Blocks[{localX,
newY, localZ}]);

                if (Swim && !blockHead) {
                        blockHead = canSwim(Chunks[{(int)chunk.x, (int)chunk.z}].Blocks[{localX,
newY, newZ}]);
                }
...
(Chunks[(int)CurrrentChunk.x][(int)CurrrentChunk.z].Blocks[RelativeX][(int)Position.y] == 0 ||
water)); bool canBreak = (BlockType == 0 &&
(Chunks[(int)CurrrentChunk.x][(int)CurrrentChunk.z].Blocks[RelativeX][(int)Position.y] != 0 &&
!water));
                ...
                if (notBedRock && (canPlace || canBreak)) {
                        std::cout << "Placing Block: " << BlockType << ", Position: " << Position.x
<< ", " << Position.y << "Chunk: " << CurrrentChunk.x << "; " << CurrrentChunk.z << std::endl; Type
= (short)Chunks[(int)CurrrentChunk.x][(int)CurrrentChunk.z].Blocks[RelativeX][(int)Position.y];
        ...
        void ShowInventor(SDL_Renderer* Renderer, int width, int height, std::vector<Slot>&
Inventory, int InventorySlot, TTF_Font* font)
        {
                SDL_SetRenderDrawColor(Renderer, 157, 76, 0, 255);
                SDL_FRect InventoryRect = { (float)(width / 2) - (BlockSize * 4), (height -
(BlockSize * 1.3f)), (BlockSize * 1.1f) * 8 + (BlockSize * .1f), (BlockSize * 1.2f) };
                ...
                for (int i = 0; i < 8; i++)
                {
                        ...
                        SDL_FRect InventoryRect = { (float)(((width / 2) - (BlockSize * 4)) +
(BlockSize * 1.1f * i) + (BlockSize * 0.1f)), (float)((height - (BlockSize * 1.3f)) + (BlockSize *
0.1f)), (float)BlockSize, (float)BlockSize }; SDL_RenderFillRect(Renderer, &InventoryRect);

                        if (Inventory[i].Type != 0)
                        {
                                SDL_SetRenderDrawColor(Renderer,
BlockDef[Inventory[i].Type].Color.r, BlockDef[Inventory[i].Type].Color.g,
BlockDef[Inventory[i].Type].Color.b, 1); SDL_FRect BlockRect = { ((width / 2) - (BlockSize * 4)) +
(BlockSize * 1.1f * i) + (BlockSize * 0.3f), (height - (BlockSize * 1.3f)) + (BlockSize * 0.3f),
BlockSize * 0.6f, BlockSize * 0.6f }; SDL_RenderFillRect(Renderer, &BlockRect);
                                //Add be a number
...
text.c_str(), sizeof(text.c_str()), White); SDL_Texture* texture =
SDL_CreateTextureFromSurface(Renderer, surface); SDL_DestroySurface(surface); SDL_FRect dstRect{
(float)(((width / 2) - (BlockSize * 4)) + (BlockSize * 1.1f * i) + (BlockSize * 0.8f)),
(float)((height - (BlockSize * 1.1f)) + (BlockSize * 0.4f)), BlockSize * 0.5f , BlockSize * 0.5f};
        ...
        void SimulateWater(int chunkIndex) {
                ...
                while (!q.empty()) {
                        ...
                        // Move down
                        if (y - 1 >= 0 && (Chunks[chunk].Blocks[localX][y - 1] == 0 ||
Chunks[chunk].Blocks[localX][y - 1] == 5)) { Chunks[chunk].Blocks[localX][y - 1] = 5;

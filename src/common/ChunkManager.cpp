...
ChunkPrefab &ChunkManager::get_chunk(Vector3 key) {
  ...
  if (this->Chunks.find(key) == this->Chunks.end()) {
    // std::cout << "Requesting chunk at: " << key.x << ", " << key.z <<
    // std::endl;
    ...
    if (this->autoGenerate) {
      std::string filename = "saves/chunk_" + std::to_string((int)key.x) + "_" +
                             std::to_string((int)key.z) + ".dat";
      std::ifstream inFile(filename, std::ios::binary);

      if (inFile.is_open()) {
        // std::cout << "Loading chunk from " << filename << std::endl;
        inFile.read(reinterpret_cast<char *>(newChunk.Blocks.data()),
                    newChunk.Blocks.size() * sizeof(int));
        inFile.close();
        newChunk.RebuildMesh();
        newChunk.isLoaded = true;
      } else {
        LOG_INFO(std::string("Generating chunk at: ") +
                 std::to_string((int)key.x) + ", " +
                 std::to_string((int)key.z));
        ...
        if (outFile.is_open()) {
          outFile.write(reinterpret_cast<const char *>(newChunk.Blocks.data()),
                        newChunk.Blocks.size() * sizeof(int));
          outFile.close();
        } else {
          LOG_ERROR(std::string("Failed to save chunk to ") + filename);
        }
      }
    }

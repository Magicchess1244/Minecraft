...
float BasicPerlinNoise(float xPos, float yPos, float zPos) {
  ...
  // Gradients at corners - FIXED: was commented out causing all zeros!
  Vector3 gradients[8];
  for (int i = 0; i < 2; ++i) {
    for (int j = 0; j < 2; ++j) {
      for (int k = 0; k < 2; ++k) {
        ...
        // Generate pseudo-random gradient from position and seed
        uint32_t hash = SeededHash(&Position, sizeof(Position), seed);
        float angle1 = (hash & 0xFFFF) * (360.0f / 65536.0f);
        float angle2 = ((hash >> 16) & 0xFFFF) * (180.0f / 65536.0f);

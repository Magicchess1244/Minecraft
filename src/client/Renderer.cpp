...
constexpr int Maxfaces = 30000;
constexpr Uint32 vertexSize = sizeof(Vertex) * 4 * Maxfaces;
constexpr Uint32 indexSize = sizeof(Uint32) * 6 * Maxfaces;
...
void Renderer::RenderChunk(const ChunkPrefab& chunk, Player& player, int NumChunk) {
    ...
    std::cout << "=== RenderChunk " << NumChunk << " ===" << std::endl;
    std::cout << "Input faces from chunk: " << chunk.allFaces.size() << std::endl;
    ...
    for (int i = -RenderDistance; i <= RenderDistance; i++) {
        for (int j = -RenderDistance; j <= RenderDistance; j++) {
            ...
            threads.emplace_back(&Renderer::RenderChunk, this,
                                 std::ref(chunkManager.get_chunk(Chunk)), std::ref(player), threadIndex);
...
void Renderer::MainRenderLoop(
    ...
    while (SDL_PollEvent(&this->event)) {
        switch (this->event.type) {
            ...
            case SDL_EVENT_WINDOW_RESIZED:
                // Update screen size
                this->Width = (unsigned int)this->event.window.data1;
                this->Height = (unsigned int)this->event.window.data2;
    ...
    this->pass = SDL_BeginGPURenderPass(this->cmdRender, &colorInfo, 1, &depth_target_info);
    ...
    for (auto& mesh : this->Terrain) {
        ...
        */
        SDL_BindGPUVertexBuffers(this->pass, 0, &mesh.VertexBuffer, (RenderDistance * 2 + 1)*(RenderDistance * 2 + 1) );
